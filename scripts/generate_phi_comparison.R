...
cat(sprintf("GRID:       phi=%.2f  beta=%.3f  [%.2fs]\n", fg$phi_opt, fg$beta_opt["x1"], tg))
cat(sprintf("CONTINUOUS: phi=%.2f (SE %.2f)  beta=%.3f  [%.2fs]\n", fd$phi_opt, sqrt(fd$cov["phi","phi"]), fd$beta_opt["x1"], td))
## profile-deviance figure with continuous estimate overlaid
prof <- data.frame(phi=fg$all_para$phi, dev=-2*(fg$all_para$value-max(fg$all_para$value)))
g <- ggplot(prof, aes(phi, dev)) +
  ...
  labs(title="Estimating the spatial scale phi",
       subtitle="Blue: grid profile deviance   Red: continuous estimate (+/-95% CI)",
       x=expression(phi), y="Profile deviance") + theme_minimal(base_size=12)
ggsave("vignettes/figures/phi_compare.png", g, width=6, height=4, dpi=120)
cat("phi figure done\n")
